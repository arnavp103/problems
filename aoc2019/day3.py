...
def parse(wire: str) -> Set[complex]:
	coords = set()
	pos = 0 + 0j
...
def manhattan_distance(pos: complex) -> int:
	return int(abs(pos.real) + abs(pos.imag))

def closest_intersection(input: str) -> int:
	wire1, wire2 = input.splitlines()
	coords1 = parse(wire1)
	coords2 = parse(wire2)
	intersections = coords1.intersection(coords2)
	return min(manhattan_distance(pos) for pos in intersections)

print(closest_intersection(input_1))

# part 2

def parse2(wire: str) -> Tuple[Set[complex], dict[complex, int]]:
	coords = set()
	pos = 0 + 0j
	steps = 0
	coords_steps: dict[complex, int] = {}
	for move in wire.split(','):
		direction = move[0]
		distance = int(move[1:])
		for _ in range(distance):
			...
			steps += 1
			coords.add(pos)
			coords_steps[pos] = min(coords_steps.get(pos, steps), steps)
	return coords, coords_steps

def closest_intersection2(input: str) -> int:
	wire1, wire2 = input.splitlines()
	coords1, coords_steps1 = parse2(wire1)
	coords2, coords_steps2 = parse2(wire2)
	intersections = coords1.intersection(coords2)
	return min(coords_steps1[pos] + coords_steps2[pos] for pos in intersections)

print(closest_intersection2(input_1))
