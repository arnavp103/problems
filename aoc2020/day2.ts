...
const input1 = `13-15 c: cqbhncccjsncqcc
2-3 v: zvdvfd
...
function parse_input(
	input: string
): [[number, number, string], { [key: string]: number }][] {
	...
	for (const line of lines) {
		...
		const bounds = [parseInt(min), parseInt(max), letter];
		const counts = {};
...
function part1(input: string): number {
	...
	let valid = 0;
	for (const [bounds, counts] of data) {
		const [min, max, letter] = bounds;
...
function part2(input: string): number {
	const data = parse_input(input);
	const pws = input.split("\n").map(x => x.split(": ")[1]);
	let valid = 0;
	let index = 0;
	for (const [bounds, _counts] of data) {
		const [min, max, letter] = bounds;
		// index is 1-based
		let at_least_one = false;
		if (pws[index][min - 1] === letter) {
			at_least_one = true;
		}
		if (pws[index][max - 1] === letter) {
