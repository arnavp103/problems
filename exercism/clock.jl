...
function Base.show(io::IO, c::Clock)
    print(io, "\"", string(c.hour, pad=2), ":",  string(c.minute, pad=2), "\"")
end

function Base.:+(c::Clock, m::Dates.Minute)
...
function Base.isless(c1::Clock, c2::Clock)
    c1.hour < c2.hour || (c1.hour == c2.hour && c1.minute < c2.minute)
end
